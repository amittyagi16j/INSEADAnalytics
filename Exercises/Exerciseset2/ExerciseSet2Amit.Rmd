...
We don't need any code to answer this, as we can just see them above. If we want to automatically answer this we can do this:

answer = Rotated_Factors[,1][!is.na(Rotated_Factors[,1])]
names(answer) <- rownames(Rotated_Factors)[!is.na(Rotated_Factors[,1])]
They are: r answer.

Question 3:

Again, no coding needed here! The components ("key risk factors") seem to be: fixed income, FX, equities, equities (again - clearly they matter), Europe related FX, oil, industrial metals, agricultural commodities, precious metals, Asia equities (e.g. Japan).

Question 4:

Let's just re-use the code above. See also this example of how to efficiently generate long reports by automatically re-using what you wrote before (and source code).

Note: code chunks also have names (e.g "Fig1"). You should not use the same code chunk name more than once in the file. Hence below we removed the names of the code chunks (which anyway we do not use in this case - but in general they may be useful as references).

Just in case we will need the variables we created also later, let's save them somewhere (with an "_orig" name) and then at the end of this question "get them back" (this is just one simple way to do this - a better way is to use what is called environments, but that is more advanced).

futures_data_orig = futures_data
Variance_Explained_Table_results_orig = Variance_Explained_Table_results
Variance_Explained_Table_orig = Variance_Explained_Table
eigenvalues_orig = eigenvalues
corused_orig = corused
Rotated_Results_orig = Rotated_Results
Rotated_Factors_orig = Rotated_Factors
Now we do first the pre-crisis years. We use the same name for the data, so we can later copy the whole code and make minor changes (see the crisis-years analysis below):

# THIS IS ALL WE CHANGE! NOTE WE HAVE STORED THE ORIGINAL DATA IN futures_data_orig. 
futures_data = futures_data_orig[as.Date(rownames(futures_data_orig)) > "2002-11-01" & as.Date(rownames(futures_data_orig)) < "2007-10-01", ]
...
What is the main risk factor (e.g. first principal component) during the specific crisis? How about in "normal times"? Is it commodities (e.g. oil), fixed income, or equities? How are the relative weights in the principal components changing across different periods? Is long-term or short-term bonds that "matter" more? Are the crisis of 2000, 2007, and possibly today, driven by the same factors?
<br>
<br>
<br>
<br>
...
The code computes the moving average returns of each future for different rolling window sizes (in this case for the window lengths of r MAfreq days), lets those "windows" vote (with the sign of the returns in that window), and then "buys" futures for which the total vote is positive and "shorts" those for which the total vote is negative.
<br>
<br>
<br>
<br>
